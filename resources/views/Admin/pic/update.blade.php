...
<div class="fixed inset-0 z-50 overflow-hidden uppercase tracking-tight" x-data="{ open: true }" x-show="open" x-cloak>
    ...
    <div class="fixed inset-y-0 right-0 pl-10 max-w-full flex">
        <div class="w-screen max-w-md shadow-2xl"
            ...
            <form action="{{ route('admin.pic.update', $pic->id) }}" method="POST" class="h-full flex flex-col bg-white font-bold">
                @csrf
                @method('PUT')
                <div class="flex-1 h-0 overflow-y-auto">
                    <!-- Drawer Header -->
                    <div class="px-8 py-6 bg-white border-b border-gray-100 flex items-center justify-between sticky top-0 z-10">
                        <h2 class="text-lg font-bold text-[#0B224E]">Form Update Photographer</h2>
                    ...
                    <div class="px-8 py-10 space-y-8 font-normal">
                        ...
                        <div class="space-y-2 uppercase tracking-widest">
                            ...
                            <input type="text" name="name" id="name" placeholder="Tulis Nama Photographer" value="{{ old('name', $pic->name) }}"
                        ...
                        <div class="space-y-2 uppercase tracking-widest">
                            ...
                            <input type="text" name="phone" id="phone" placeholder="Contoh: 08123456789" value="{{ old('phone', $pic->phone) }}"
                        ...
                        <div class="space-y-2 uppercase tracking-widest">
                            ...
                            <select name="location_id" id="location_id" 
                                ...
                                @foreach ($lokasi as $item)
                                    <option value="{{ $item->id }}" {{ old('location_id', $pic->location_id) == $item->id ? 'selected' : '' }}>{{ $item->name }} - {{ $item->Kabupaten }}</option>
                                @endforeach
                            </select>
