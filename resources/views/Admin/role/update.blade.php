@extends('Admin.role.index')

@section('drawer')
<div class="fixed inset-0 z-50 overflow-hidden uppercase tracking-tight" x-data="{ open: true }" x-show="open" x-cloak>
    <!-- Backdrop -->
    <div class="absolute inset-0 bg-black/20 backdrop-blur-[2px] transition-opacity" @click="window.location.href='{{ route('admin.role.index') }}'"></div>
    ...
    <div class="fixed inset-y-0 right-0 pl-10 max-w-full flex">
        <div class="w-screen max-w-md shadow-2xl"
            ...
            <form action="{{ route('admin.role.update', $role_edit->id) }}" method="POST" class="h-full flex flex-col bg-white">
                @csrf
                @method('PUT')
                <div class="flex-1 h-0 overflow-y-auto">
                    <!-- Drawer Header -->
                    <div class="px-8 py-6 bg-white border-b border-gray-100 flex items-center justify-between sticky top-0 z-10">
                        <h2 class="text-lg font-bold text-[#0B224E]">Form Update Role</h2>
                        <button type="button" @click="window.location.href='{{ route('admin.role.index') }}'" class="p-2 text-gray-400 hover:text-[#0B224E] transition-colors rounded-lg hover:bg-gray-50">
                    ...
                    <div class="px-8 py-10 space-y-8">
                        <!-- Input Group -->
                        <div class="space-y-2">
                            <label for="name" class="block text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em]">Nama Role</label>
                            <input type="text" name="name" id="name" placeholder="Tulis Nama Role" value="{{ old('name', $role_edit->name) }}"
                ...
                <!-- Drawer Footer -->
                <div class="flex-shrink-0 px-8 py-6 border-t border-gray-50 flex items-center gap-3 bg-gray-50/30">
                    <button type="button" @click="window.location.href='{{ route('admin.role.index') }}'"
